describe('OPENER network runtime', () => {
  ...
  const receive = function (protocol, expects, done) {
    const listener = function (message) {
      ...
      if (!msg.fromClient) { return; }
      if (msg.payload.protocol !== protocol) { return; }
  ...
  describe('Component protocol', () => describe('on requesting a component list', () => it('should receive some known components', (done) => {
    let received = 0;
    const listener = function (message) {
      const originalMessage = JSON.parse(message.data);
      if (!originalMessage.fromClient) { return; }
      const msg = originalMessage.payload;
